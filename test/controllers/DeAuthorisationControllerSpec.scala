...
import connectors.DeAuthorisationConnector
...
class DeAuthorisationControllerSpec
  ...
  private val mockDeAuthorisationConnector = mock[DeAuthorisationConnector]
  ...
  private val modules: Seq[GuiceableModule] =
    Seq(
      ...
      bind[DeAuthorisationConnector].toInstance(mockDeAuthorisationConnector)
    )
  ...
  private val controller: DeAuthorisationController =
    application.injector.instanceOf[DeAuthorisationController]

  before {
    reset(mockDeAuthorisationConnector, mockAuthConnector)
  ...
  "deAuthorisePsp" must {
    "return OK when valid response from IF" in {
      ...
      when(mockDeAuthorisationConnector.deAuthorisePsp(any(), any())(any(), any(), any()))
