...
class PspAuthActionSpec extends PlaySpec with GuiceOneAppPerSuite with BeforeAndAfterEach {

  private type RetrievalsType = Enrolments ~ Option[String]

  class Harness(authAction: PspAuthAction) {
    ...
    "when the user is logged in and has a PODS enrolment" must {
      ...
      "must succeed" in {
        ...
        running(app) {
          ...
          val action = new PspAuthAction(mockAuthConnector, bodyParsers)
    ...
    "when the user is not logged in" must {
      ...
      "must return Unauthorized" in {
        ...
        running(app) {
          ...
          val authAction = new PspAuthAction(new FakeFailingAuthConnector(new MissingBearerToken), bodyParsers)
