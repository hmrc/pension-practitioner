...
class PspSchemeActionSpec extends PlaySpec with MockitoSugar with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  private val authRequest = PspAuthRequest[AnyContent](FakeRequest("", ""), PspId(AuthUtils.pspId),  AuthUtils.externalId)
