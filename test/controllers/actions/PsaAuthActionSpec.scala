...
class PsaAuthActionSpec extends PlaySpec with GuiceOneAppPerSuite with BeforeAndAfterEach {

  private type RetrievalsType = Enrolments ~ Option[String]

  class Harness(authAction: PsaAuthAction) {
    ...
    "when the user is logged in and has a PODS enrolment" must {
      ...
      "must succeed" in {
        ...
        running(app) {
          ...
          val action = new PsaAuthAction(mockAuthConnector, bodyParsers)
    ...
    "when the user is not logged in" must {
      ...
      "must return Unauthorized" in {
        ...
        running(app) {
          ...
          val authAction = new PsaAuthAction(new FakeFailingAuthConnector(new MissingBearerToken), bodyParsers)
