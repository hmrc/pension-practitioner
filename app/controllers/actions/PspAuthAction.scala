...
case class PspAuthRequest[A](request: Request[A], pspId: PspId, externalId: String) extends WrappedRequest[A](request)

class PspAuthAction @Inject()(
                            override val authConnector: AuthConnector,
                            val parser: BodyParsers.Default
                          )(implicit val executionContext: ExecutionContext)
  extends ActionBuilder[PspAuthRequest, AnyContent]
    with ActionFunction[Request, PspAuthRequest]
  ...
  override def
  invokeBlock[A](request: Request[A], block: PspAuthRequest[A] => Future[Result]): Future[Result] =
    invoke(request, block)(HeaderCarrierConverter.fromRequest(request))

  private def invoke[A](request: Request[A], block: PspAuthRequest[A] => Future[Result])
               (implicit hc: HeaderCarrier): Future[Result] = {
    authorised(Enrolment(PSPEnrolmentKey)).retrieve(Retrievals.authorisedEnrolments and Retrievals.externalId) {
      case enrolments ~ Some(externalId) =>
        ...
        pspId match {
          ...
          case Some(pspId) => block(PspAuthRequest(request, PspId(pspId), externalId))
        }
